...
class ChannelWiringTests(unittest.TestCase):
    def test_generate_answer_used_by_all_channels(self):
        ...
        self.assertIn("build_system_context", main_src)
        self.assertIn("guess_reply_lang", main_src)
        self.assertIn("build_mandatory_profile_prefix", main_src)
        self.assertIn("compose_profile_first_answer", main_src)
        self.assertIn("answer_text = compose_profile_first_answer(prefix, llm_text)", main_src)
        self.assertIn("prefix = build_mandatory_profile_prefix(q, resolved_lang)", main_src)
